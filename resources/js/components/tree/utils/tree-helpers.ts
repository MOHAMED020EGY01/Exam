 ...
 */
export function filterTree(nodes: any, query: string): TreeNodeData[] {
  ...
  return safeNodes
    .map((node: any) => {
      if (!node) return null;
      const matchesSelf = node.label?.toLowerCase().includes(lowerQuery) || false;
      ...
      if (safeChildren.length > 0) {
        const filteredChildren = filterTree(safeChildren, query);
