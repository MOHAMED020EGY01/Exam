...
interface MoveModalProps {
    ...
    nodeToMove: TreeNodeType | null; // The tree node we want to move
    ...
    const options = useMemo(() => {
        if (isExam) {
            ...
            const currentCourseId = String(
                nodeToMove.parentId?.replace("course-", ""),
            );
            return courses
            ...
            const originalData = nodeToMove.originalData as {
            ...
            <DialogContent className="sm:max-w-md modal-move flex flex-col p-6 overflow-hidden">
                <DialogHeader className="shrink-0 pb-2 border-b">
                    ...
                    <DialogDescription className="text-xs text-muted-foreground mt-1">
                        Choose a new target location to migrate this item. Gaps
                ...
                <div className="shrink-0 flex items-center gap-3 p-3 bg-muted/40 rounded-lg border border-dashed text-xs my-3 select-none">
                    <div className="flex-1 min-w-0">
                        ...
                        <span className="font-semibold block truncate text-foreground/90">
                            {nodeToMove.label}
                        </span>
                    </div>
