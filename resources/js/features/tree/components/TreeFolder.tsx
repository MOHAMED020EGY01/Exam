 ...
 * - Lucide icons
 * - useTreeContext, useClipboardContext
 *
 */
...
const TreeFolderInner: React.FC<TreeFolderProps> = ({
    ...
    // Pull all action callbacks from tree context, clipboard from clipboard context
    const { actions } = useTreeContext();
    ...
    // For memoization
    const clipboardId = useMemo(() => clipboard?.id, [clipboard?.id]);
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                            ...
                            {isCourse && (
                                <>
                                    ...
                                    {clipboard?.type === "exam" && (
                                        <DropdownMenuItem
                                            onClick={() =>
                                                clipboardActions.paste(node)
                                            }
                                        >
                                            <Clipboard className="icon-sm mr-2 icon-indigo" />
                                            Paste Exam
                                        </DropdownMenuItem>
                                    )}
                            ...
                            {isExam && (
                                <>
                                    ...
                                    <DropdownMenuItem
                                        onClick={() =>
                                            clipboardActions.duplicate(node)
                                        }
                                    >
                                        <CopyPlus className="icon-sm mr-2" />
                                        Duplicate
                                    </DropdownMenuItem>

                                    <DropdownMenuItem
                                        onClick={() =>
                                            clipboardActions.move(node)
                                        }
                                    >
                                        <Move className="icon-sm mr-2" />
                                        Move Exam
                                    </DropdownMenuItem>
...
export const TreeFolder = React.memo(
    ...
    (prev, next) =>
        ...
        prev.node.id === (next as any).__clipboardId, // We'll use node.id as the memo key
);
