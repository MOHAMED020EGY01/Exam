    ...
    return (
        <div className="relative w-full flex items-center group">
            ...
            <div className="absolute left-3 flex items-center pointer-events-none select-none">
                {isPending ? (
                    <Loader2 className="icon-sm text-primary animate-spin" />
                ) : (
                    <Search className="icon-sm text-muted-foreground/60 transition-colors group-focus-within:text-primary" />
                )}
            </div>
            ...
            <div className="absolute right-1 select-none">
                <DropdownMenu>
                    ...
                    <DropdownMenuContent
                        align="end"
                        className="w-36 bg-popover rounded-lg border border-border shadow-md"
                    >
                        <DropdownMenuRadioGroup
                            value={scope}
                            onValueChange={(val) =>
                                setScope(val as SearchScope)
                            }
                        >
                            <DropdownMenuRadioItem
                                value="all"
